...
def test_adding_files(new_repo: git.Repo):
    initially_unstaged_files = llmit.repo.get_untracked_and_modified_files(new_repo)

    assert len(initially_unstaged_files) > 0

    llmit.repo.add_files(new_repo, interactive=False)

    now_unstaged_files = llmit.repo.get_untracked_and_modified_files(new_repo)
    ...
    assert len(llmit.repo.get_untracked_and_modified_files(new_repo)) == 0


def test_adding_files_with_one_removed(new_repo: git.Repo):
    root_dir = pathlib.Path(new_repo.working_dir)
    all_files = llmit.repo.get_untracked_and_modified_files(new_repo)
...
def test_finding_repo_root(new_repo: git.Repo, monkeypatch):
    root_dir = pathlib.Path(new_repo.working_dir)
    assert pathlib.Path(llmit.repo.get_repo().working_dir) == root_dir
...
def test_getting_all_files(new_repo: git.Repo):
    assert len(llmit.repo.get_untracked_and_modified_files(new_repo)) == 5
    new_repo.index.add(["README.md"])
    assert len(llmit.repo.get_untracked_and_modified_files(new_repo)) == 4
...
def test_generating_diffs(new_repo: git.Repo):
    ...
    with pytest.raises(NotImplementedError):
        diff_summary = llmit.repo.generate_diffs_with_context(new_repo)
...
def test_diffs_with_deleted_file(new_repo: git.Repo):
    ...
    # now we can delete
    os.remove("README.md")
    llmit.run_cmd("git add -A")
    llmit.repo.generate_diffs_with_context(new_repo)
...
def test_deleted_files(new_repo: git.Repo):
    ...
    os.remove("README.md")
    llmit.repo.add_files(new_repo, interactive=False)
    diffs = llmit.repo.generate_diffs_with_context(new_repo)
    assert diffs != ""
